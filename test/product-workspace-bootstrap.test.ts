...
describe('PANGEA product workspace bootstrap bridge', () => {
  it('returns the Desktop-owned launch root to the trusted Harness client', async () => {
    const [main, preload, desktopBridge] = await Promise.all([
    ...
    expect(desktopBridge).toContain("ipc.invoke('pangea:product-workspace')")
    expect(desktopBridge).toContain("ipc.invoke('pangea:product-workspace-ready')")
  })
})
